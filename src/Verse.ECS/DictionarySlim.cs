...
///     3) It assumes the keys implement <see cref="IEquatable{TKey}" /> or else Equals() and they are cheap and
///     sufficient.
/// </summary>
/// <remarks>
...
public class DictionarySlim<TKey, TValue> : IReadOnlyCollection<KeyValuePair<TKey, TValue>> where TKey : IEquatable<TKey>
{
	...
	{
		Debug.Assert(_entries.Length == Count || _entries.Length == 1); // We only copy _count, so if it's longer we will miss some
		var count = Count;
...
internal static class ThrowHelper
{
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	internal static bool ThrowNotSupportedException() => throw new NotSupportedException();
}