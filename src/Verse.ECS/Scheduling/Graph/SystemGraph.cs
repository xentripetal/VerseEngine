...
public class SystemGraph
{
	...
	public IEnumerable<(NodeId, ISystem, ICondition[])> GetSystems()
	{
		for (var i = 0; i < Systems.Count; i++) {
			...
			yield return (new NodeId(i, NodeType.System, this), system, conditions);
		}
	}
	...
	public IEnumerable<(NodeId, ISystemSet, ICondition[])> GetSets()
	{
		for (var i = 0; i < SystemSets.Count; i++) {
			...
			yield return (new NodeId(i, NodeType.Set, this), set, conditions);
		}
	}
			...
			if (chained) {
				...
				foreach (var previousNode in previousNodes) {
					foreach (var currentNode in currentNodes) {
						SafeAddEdge(Dependency, new Edge<NodeId>(previousNode, currentNode));
						if (ignoreDeferred) {
	...
	{
		var id = new NodeId(Systems.Count, NodeType.System, this);
	...
	{
		var id = new NodeId(SystemSets.Count, NodeType.Set, this);
		SystemSets.Add(set);
		SystemSetIds[set] = id;
	...
	protected void UpdateGraphs(NodeId id, SubgraphInfo subgraph)
	{
		...
		foreach (var set in subgraph.Hierarchy) {
			...
			SafeAddEdge(Hierarchy, new Edge<NodeId>(setId, id));
		...
		foreach (var dependency in subgraph.Dependencies) {
			...
			SafeAddEdge(Dependency, new Edge<NodeId>(lhs, rhs));
		...
		if (subgraph.AmbiguousWith is Ambiguity.IgnoreWithSet ignoreWithSet) {
			foreach (var set in ignoreWithSet.Sets) {
				SafeAddEdge(AmbiguousWith, new Edge<NodeId>(id, SystemSetIds[set]));
			}
	...
	private static bool SafeAddEdge<T>(
		IMutableVertexAndEdgeSet<T, Edge<T>> graph,
		Edge<T> edge)
	{
	...
	public SystemSchedule BuildSchedule(World world, string scheduleName, FixedBitSet ignoredAmbiguities)
	{
	...
	public SystemSchedule UpdateSchedule(World world, SystemSchedule schedule, FixedBitSet ignoredAmbiguities, string scheduleName)
	{
		if (Uninit.Count != 0) {
		...
		var newSchedule = BuildSchedule(world, scheduleName, ignoredAmbiguities);
		...
		foreach (var edge in AmbiguousWith.Edges) {
			foreach (var source in ResolveNodeToSystems(setSystems, edge.Source)) {
				foreach (var target in ResolveNodeToSystems(setSystems, edge.Target)) {
					SafeAddEdge(graph, new Edge<NodeId>(source, target));
				}
			}
		}
		return graph;
		...
		foreach (var node in topo) {
			...
			foreach (var edge in flattened.OutEdges(node)) {
				...
				if (addSyncOnEdge) {
					...
					SafeAddEdge(syncGraph, new Edge<NodeId>(node, syncPoint));
					SafeAddEdge(syncGraph, new Edge<NodeId>(syncPoint, edge.Target));
	...
	protected BidirectionalGraph<NodeId, Edge<NodeId>> GetDependencyFlattened(Dictionary<NodeId, List<NodeId>> setSystems)
	{
		...
		foreach (var (set, systems) in setSystems) {
			...
			foreach (var (a, b) in temp) {
				SafeAddEdge(flattened, new Edge<NodeId>(a, b));
			}
			temp.Clear();
		}
	...
	public static CheckGraphResults<Tv> CheckGraph<Tv>(IBidirectionalGraph<Tv, Edge<Tv>> graph, Tv[] topologicalOrder) where Tv : notnull
	{
		...
		for (var i = 0; i < n; i++) {
			...
			foreach (var edge in graph.InEdges(topologicalOrder[i])) {
				SafeAddEdge(topSorted, new Edge<Tv>(edge.Source, edge.Target));
			}
		}
		...
		for (var indexA = n - 1; indexA >= 0; indexA--) {
			...
			foreach (var edge in topSorted.OutEdges(a)) {
				...
				if (!visited[indexB]) {
					SafeAddEdge(transitiveReduction, new Edge<Tv>(a, b));
					SafeAddEdge(transitiveClosure, new Edge<Tv>(a, b));
					...
					foreach (var c in transitiveClosure.OutEdges(b).Select(x => x.Target)) {
						...
						if (!visited[indexC]) {
							visited.Set(indexC);
							SafeAddEdge(transitiveClosure, new Edge<Tv>(a, c));
	...
	private string GetConflictsErrorMessage(World world, List<(NodeId, NodeId, ulong[])> ambiguities)
	{
		...
		foreach (var (nameA, nameB, conflicts) in ambiguities) {
			...
			if (conflicts.Length != 0) {
				message += $"    conflict on: {string.Join(", ", conflicts.Select(c => world.Registry.GetComponent(c).Name))}\n";
			} else {
