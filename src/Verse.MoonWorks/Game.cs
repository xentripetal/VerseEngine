...
public abstract class Game
{
	...
	private TimeSpan accumulatedDrawTime = TimeSpan.Zero;
	private TimeSpan accumulatedUpdateTime = TimeSpan.Zero;
	...
	)
	{
		...
		// if in debug mode use the content dir, else use the bin dir
#if DEBUG
	...
	protected abstract void Draw(double delta, double alpha);
	...
	private void Tick()
	{
		...
		if (FramePacingSettings.Mode != FramePacingMode.Uncapped) {
			...
			while (accumulatedDrawTime + worstCaseSleepPrecision < capTimespan) {
				Thread.Sleep(1);
				var timeAdvancedSinceSleeping = AdvanceElapsedTime();
		...
		if (!quit) {
			...
			while (accumulatedUpdateTime >= FramePacingSettings.Timestep) {
				var delta = gameTimer.Elapsed.TotalSeconds - _lastTick.TotalSeconds;
				if (_firstTick) {
					_firstTick = false;
					delta = FramePacingSettings.Timestep.TotalSeconds;
				}
			...
			var alpha = FramePacingSettings.Mode == FramePacingMode.LatencyOptimized ? 0 : accumulatedUpdateTime / FramePacingSettings.Timestep;


			var drawDelta = (gameTimer.Elapsed - _lastDraw).TotalSeconds;
			Draw(drawDelta, alpha);
			_lastDraw = gameTimer.Elapsed;
	...
	private void HandleTextInput(SDL.SDL_TextInputEvent evt)
	{
		...
		unsafe {
			var bytes = MeasureStringLength(evt.text);
			if (bytes > 0) {
	...
	private void UpdateEstimatedSleepPrecision(TimeSpan timeSpentSleeping)
	{
		...
		if (timeSpentSleeping >= worstCaseSleepPrecision) {
			...
			for (var i = 0; i < previousSleepTimes.Length; i++) {
