...
public static class MathHelper
{
	...
	public static Fix64 Quantize(Fix64 value, Fix64 step) => Fix64.Floor(value / step) * step;

	/// <summary>
	///     Rescales a value within a given range to a new range.
	/// </summary>
	public static float Normalize(short value, short min, short max, short newMin, short newMax) => (float)(value - min) * (newMax - newMin) / (max - min) + newMin;
	...
	public static float Approach(float start, float end, float change) => start < end ? System.Math.Min(start + change, end) : System.Math.Max(start - change, end);
	...
	/// <param name="end">End value.</param>
	/// <param name="change">Change value.</param>
	public static Fix64 Approach(Fix64 start, Fix64 end, Fix64 change) => start < end ? Fix64.Min(start + change, end) : Fix64.Max(start - change, end);
