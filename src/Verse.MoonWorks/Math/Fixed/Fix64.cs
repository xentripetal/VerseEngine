...
namespace Verse.MoonWorks.Math.Fixed;
...
public struct Fix64 : IEquatable<Fix64>, IComparable<Fix64>
{
	...
	/// </summary>
	public static Fix64 FromFraction(int numerator, int denominator) => new Fix64(numerator) / new Fix64(denominator);

	public static Fix64 FromRawValue(long value) => new Fix64(value);

	/// <summary>
	///     Gets the fractional component of this Fix64 value.
	/// </summary>
	public static Fix64 Fractional(Fix64 number) => new Fix64(number.RawValue & 0x00000000FFFFFFFF);
	...
	public static int Sign(Fix64 value) => value.RawValue < 0 ? -1 :
		value.RawValue > 0                                    ? 1 :
		                                                        0;
	...
	public static Fix64 Floor(Fix64 value) =>
		...
		new Fix64((long)((ulong)value.RawValue & 0xFFFFFFFF00000000));

	/// <summary>
	///     Returns the smallest integral value that is greater than or equal to the specified number.
	/// </summary>
	public static Fix64 Ceiling(Fix64 value) => value.IsFractional ? Floor(value) + One : value;
	...
	public static Fix64 Lerp(Fix64 value1, Fix64 value2, Fix64 amount) => value1 + (value2 - value1) * amount;

	/// <summary>
	///     Rescales a value within a given range to a new range.
	/// </summary>
	public static Fix64 Normalize(Fix64 value, Fix64 min, Fix64 max, Fix64 newMin, Fix64 newMax) => (value - min) * (newMax - newMin) / (max - min) + newMin;

	/// <summary>
	///     Returns Floor(value / step) * step. Nearest multiple of step to a specified value
	/// </summary>
	public static Fix64 Step(Fix64 value, Fix64 step) => Floor(value / step) * step;
	...
	/// </summary>
	public static Fix64 ToRadians(Fix64 degrees) => degrees * (Pi / new Fix64(180));

	/// <summary>
	///     Converts radians to degrees.
	/// </summary>
	public static Fix64 ToDegrees(Fix64 radians) => radians * (new Fix64(180) / Pi);
	...
	public static Fix64 Sin(Fix64 x)
	{
		...
		var nearestValue = new Fix64(Fix64Lut.Sin[flipHorizontal ? Fix64Lut.Sin.Length - 1 - (int)roundedIndex : (int)roundedIndex]);
		var secondNearestValue = new Fix64(Fix64Lut.Sin[flipHorizontal ? Fix64Lut.Sin.Length - 1 - (int)roundedIndex - Sign(indexError) : (int)roundedIndex + Sign(indexError)]);
	...
	public static Fix64 Atan2(Fix64 y, Fix64 x)
	{
		...
		// Deal with overflow
		if (One + FromFraction(28, 100) * z * z == MaxValue) {
			return y < Zero ? -PiOver2 : PiOver2;
		}

		if (Abs(z) < One) {
			atan = z / (One + FromFraction(28, 100) * z * z);
			if (xl < 0) {
				if (yl < 0) {
					return atan - Pi;
				}
			...
			atan = PiOver2 - z / (z * z + FromFraction(28, 100));
			if (yl < 0) {
				return atan - Pi;
			}
		}
	...
	public static Fix64 operator +(Fix64 x, int y) => x + new Fix64(y);

	public static Fix64 operator +(int x, Fix64 y) => new Fix64(x) + y;
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private static Fix64 FastSub(Fix64 x, Fix64 y) => new Fix64(x.RawValue - y.RawValue);
	...
	public static Fix64 operator %(Fix64 x, Fix64 y) => new Fix64(
	...
	public static Fix64 operator -(Fix64 x) => x.RawValue == MIN_VALUE ? MaxValue : new Fix64(-x.RawValue);
	...
	public static bool operator >(Fix64 x, int y) => x > (Fix64)y;
	...
	public static explicit operator float(Fix64 value) => (float)value.RawValue / ONE;

	public static explicit operator Fix64(double value) => new Fix64((long)(value * ONE));

	public static explicit operator double(Fix64 value) => (double)value.RawValue / ONE;

	public static explicit operator Fix64(decimal value) => new Fix64((long)(value * ONE));

	public static explicit operator decimal(Fix64 value) => (decimal)value.RawValue / ONE;

	public int CompareTo(Fix64 other) => RawValue.CompareTo(other.RawValue);

	public override bool Equals(object obj) => obj is Fix64 fix && RawValue == fix.RawValue;
	...
	public string ToString(CultureInfo ci) => ((decimal)this).ToString("0.##########", ci);
}