using Verse.MoonWorks.Math.Fixed;

namespace Verse.MoonWorks.Math;
...
public static class Easing
{
	...
	private static float OutIn(
		Func<float, float> outFunc,
		Func<float, float> inFunc,
		float t
	)
	{
		if (t < 0.5f) {
	...
	private static Fix64 OutIn(
		Func<Fix64, Fix64> outFunc,
		Func<Fix64, Fix64> inFunc,
		Fix64 t
	)
	{
		if (t < HALF) {
	...
	)
	{
		...
		if (time >= attackDuration && time < attackDuration + holdDuration) {
			return hold;
		}
		// time >= attackDuration + holdDuration
	...
	public static float Interp(float start, float end, float time, float duration, Func<float, float> easingFunc) => Lerp(start, end, easingFunc(time / duration));

	public static float Interp(float start, float end, float time, float duration, Function.Float easingFunc) => Interp(start, end, time, duration, Function.Get(easingFunc));

	public static Fix64 Lerp(Fix64 start, Fix64 end, Fix64 time) => start + (end - start) * time;

	public static Fix64 Interp(Fix64 start, Fix64 end, Fix64 time, Fix64 duration, Func<Fix64, Fix64> easingFunc) => Lerp(start, end, easingFunc(time / duration));

	public static Fix64 Interp(Fix64 start, Fix64 end, Fix64 time, Fix64 duration, Function.Fixed easingFunc) => Interp(start, end, time, duration, Function.Get(easingFunc));
	...
	{
		if (t < 0.5f) {
			return (1 - MathF.Sqrt(1 - 2 * t * (2 * t))) / 2;
		}
		var x = -2 * t + 2;
		return (MathF.Sqrt(1 - x * x) + 1) / 2;
	}
	...
	public static float OutBack(float t) => 1 + C3 * (t - 1) * (t - 1) * (t - 1) + C1 * (t - 1) * (t - 1);
	...
	public static Fix64 Linear(Fix64 t) => t;

	// QUADRATIC

	public static Fix64 InQuad(Fix64 t) => t * t;

	public static Fix64 OutQuad(Fix64 t) => 1 - (1 - t) * (1 - t);
	...
	public static Fix64 InCubic(Fix64 t) => t * t * t;
	...
	public static Fix64 InQuart(Fix64 t) => t * t * t * t;
	...
	public static Fix64 InQuint(Fix64 t) => t * t * t * t * t;
	...
	public static Fix64 InSine(Fix64 t) => 1 - Fix64.Cos(t * Fix64.Pi / 2);

	public static Fix64 OutSine(Fix64 t) => Fix64.Sin(t * Fix64.Pi / 2);

	public static Fix64 InOutSine(Fix64 t) => -(Fix64.Cos(Fix64.Pi * t) - 1) / 2;

	public static Fix64 OutInSine(Fix64 t) => OutIn(OutSine, InSine, t);

	// CIRCULAR

	public static Fix64 InCirc(Fix64 t) => 1 - Fix64.Sqrt(1 - t * t);

	public static Fix64 OutCirc(Fix64 t) => Fix64.Sqrt(1 - (t - 1) * (t - 1));
	...
	public static Fix64 InBack(Fix64 t) => FIXED_C3 * t * t * t - FIXED_C1 * t * t;

	public static Fix64 OutBack(Fix64 t) => 1 + FIXED_C3 * (t - 1) * (t - 1) * (t - 1) + FIXED_C1 * (t - 1) * (t - 1);
	...
	public static Fix64 InBounce(Fix64 t) => 1 - OutBounce(1 - t);
	...
	public static Fix64 OutBounce(Fix64 t)
	{
		...
		if (t < Fix64.FromFraction(5, 2) / FIXED_D1) {
	...
	public static class Function
	{
		...
		private static readonly Dictionary<Float, Func<float, float>> FloatLookup = new Dictionary<Float, Func<float, float>> {
			{ Float.Linear, Linear },
			{ Float.InQuad, InQuad },
		...
		private static readonly Dictionary<Fixed, Func<Fix64, Fix64>> FixedLookup = new Dictionary<Fixed, Func<Fix64, Fix64>> {
			{ Fixed.Linear, Linear },
			{ Fixed.InQuad, InQuad },
		...
		public static Func<float, float> Get(Float functionEnum) => FloatLookup[functionEnum];

		public static Func<Fix64, Fix64> Get(Fixed functionEnum) => FixedLookup[functionEnum];
	}
}