...
public class AudioDataQoa : AudioDataStreamable
{
	...
	public static AudioDataQoa Create(AudioDevice device) => new AudioDataQoa(device);
	...
	public static unsafe Format GetFormat(TitleStorage storage, string filePath)
	{
		...
		FAudio.qoa_attributes(handle, out var channels, out var samplerate, out _, out _);
		var format = new Format {
			Tag = FormatTag.PCM,
			BitsPerSample = 16,
