...
public class AudioDataOgg : AudioDataStreamable
{
	...
	private readonly IntPtr[] AudioBuffers = new IntPtr[BUFFER_COUNT];
	...
	public static AudioDataOgg Create(AudioDevice device) => new AudioDataOgg(device);
	...
	public override unsafe void Open(ReadOnlySpan<byte> data)
	{
		...
		VorbisHandle = FAudio.stb_vorbis_open_memory(BufferDataPtr, data.Length, out var error, IntPtr.Zero);
		if (error != 0) {
		...
		for (var i = 0; i < BUFFER_COUNT; i += 1) {
	...
	protected override FAudio.FAudioBuffer OnBufferNeeded()
	{
		...
		if (filledLengthInBytes > 0) {
			return new FAudio.FAudioBuffer {
				...
				pAudioData = buffer,
				PlayLength = filledLengthInBytes /
				             Format.Channels /
	...
	private static unsafe LoadResult Load(ReadOnlySpan<byte> data)
	{
		IntPtr buffer;
		uint lengthInBytes;
		...
		fixed (void* ptr = data) {
			var filePointer = FAudio.stb_vorbis_open_memory((nint)ptr, data.Length, out var error, IntPtr.Zero);
			if (error != 0) {
