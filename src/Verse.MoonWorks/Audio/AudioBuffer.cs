...
public class AudioBuffer : AudioResource
{
	...
	/// </summary>
	public static AudioBuffer Create(AudioDevice device, Format format) => new AudioBuffer(device, format);

	/// <summary>
	///     Create a new empty AudioBuffer with a format to be specified later.
	/// </summary>
	public static AudioBuffer Create(AudioDevice device) => new AudioBuffer(device);
	...
	/// </summary>
	public void SetDataPointer(IntPtr bufferDataPtr, uint bufferDataLength, bool owns)
	{
		if (OwnsBufferData) {
			FreeBufferData();
		}
	...
	public FAudio.FAudioBuffer ToFAudioBuffer(bool loop = false) => new FAudio.FAudioBuffer {
