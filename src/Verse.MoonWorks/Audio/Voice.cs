...
public abstract unsafe class Voice : AudioResource
{
		...
		internal set {
			if (ReverbEffect != null) {
				...
				if (reverb != value) {
					...
					var outputMatrix = (float*)pMatrixCoefficients;
	...
	public void SetOutputVoice(SubmixVoice send)
	{
		OutputVoice = send;
		...
		if (ReverbEffect != null) {
			...
			var sendDesc = stackalloc FAudio.FAudioSendDescriptor[1];
			sendDesc[0].Flags = 0;
	...
	/// </summary>
	public void SetReverbEffectChain(ReverbEffect reverbEffect)
	{
	...
	private void SetPanMatrixCoefficients()
	{
	...
	{
		float doppler;
		var dopplerScale = Device.DopplerScale;
