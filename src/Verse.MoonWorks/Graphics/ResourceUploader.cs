...
public unsafe class ResourceUploader : GraphicsResource
{
	...
	private readonly List<BufferUpload> BufferUploads = [];
	private readonly List<TextureUpload> TextureUploads = [];
	...
	{
		var elementSize = (uint)Marshal.SizeOf<T>();
		var offsetInBytes = elementSize * bufferOffsetInElements;
		var lengthInBytes = (uint)(elementSize * data.Length);
	...
	{
		ImageUtils.ImageInfoFromBytes(compressedImageData, out var width, out var height, out _);
		var texture = Texture.Create2D(Device, name, width, height, format, usage);
	...
	public Texture CreateTexture2DFromCompressed(TitleStorage storage, string compressedImageFilePath, TextureFormat format, TextureUsageFlags usage) =>
		CreateTexture2DFromCompressed(Path.GetFileNameWithoutExtension(compressedImageFilePath), storage, compressedImageFilePath, format, usage);
	...
	public Texture CreateTextureFromDDS(string name, ReadOnlySpan<byte> span)
	{
		...
		for (var face = 0; face < faces; face += 1) {
	...
	public Texture CreateTextureFromDDS(TitleStorage storage, string path) =>
		CreateTextureFromDDS(Path.GetFileNameWithoutExtension(path), storage, path);

	public void SetTextureDataFromCompressed(TextureRegion textureRegion, ReadOnlySpan<byte> compressedImageData)
	{
		var pixelData = ImageUtils.GetPixelDataFromBytes(compressedImageData, out _, out _, out var sizeInBytes);
		var pixelSpan = new ReadOnlySpan<byte>(pixelData, (int)sizeInBytes);
	...
	{
		var lengthInBytes = (uint)(Marshal.SizeOf<T>() * span.Length);
	...
	private uint RoundToAlignment(uint value, uint alignment) => alignment * ((value + alignment - 1) / alignment);
	...
	private record struct BufferUpload(uint Offset, BufferRegion BufferRegion, bool Cycle);
	private record struct TextureUpload(uint Offset, TextureRegion TextureRegion, bool Cycle);
}