...
public unsafe class TextBatch : GraphicsResource
{
	...
	private readonly int ChunkDataSize;
	private readonly TransferBuffer ChunkDataTransferBuffer;
	private uint CurrentFontIndex;

	private readonly TextureSamplerBinding[] FontTextureBindings = new TextureSamplerBinding[4];
	...
	private readonly TransferBuffer VertexTransferBuffer;
	...
	public Buffer VertexBuffer { get; protected init; }
	public Buffer IndexBuffer { get; protected init; }
	...
	{
		if (!FontIndices.TryGetValue(font, out var fontIndex)) {
		...
		fixed (char* chars = text) {
			Encoding.UTF8.GetBytes(chars, text.Length, StringBytes, byteCount);
