...
public record struct Color(byte R, byte G, byte B, byte A = 255)
{
	...
	/// </summary>
	public static Color LightSteelBlue => new Color(179, 196, 222);
	...
	/// <returns>A <see cref="Color" /> which contains premultiplied alpha data.</returns>
	public static Color FromNonPremultiplied(byte r, byte g, byte b, byte a) => new Color(
		r * a / 255,
		g * a / 255,
		b * a / 255,
		a
	...
	public static Color FromHSV(float r, float g, float b)
	{
		...
		var hueSlice = 6 * r; // [0, 6)
		var hueSliceInteger = MathF.Floor(hueSlice);

		// In [0,1) for each hue slice
		var hueSliceInterpolant = hueSlice - hueSliceInteger;

		var tempRGB = new Vector3(
			b * (1f - g),
		...
		//
		var isOddSlice = hueSliceInteger % 2f;                          // 0 if even (slices 0, 2, 4), 1 if odd (slices 1, 3, 5)
		var threeSliceSelector = 0.5f * (hueSliceInteger - isOddSlice); // (0, 1, 2) corresponding to slices (0, 2, 4) and (1, 3, 5)

		var scrollingRGBForEvenSlices = new Vector3(b, tempRGB.Z, tempRGB.X); // (V, Temp Blue, Temp Red) for even slices (0, 2, 4)
		var scrollingRGBForOddSlices = new Vector3(tempRGB.Y, b, tempRGB.X);  // (Temp Green, V, Temp Red) for odd slices (1, 3, 5)
		var scrollingRGB = Vector3.Lerp(scrollingRGBForEvenSlices, scrollingRGBForOddSlices, isOddSlice);

		var IsNotFirstSlice = float.Clamp(threeSliceSelector, 0f, 1f);       // 1 if NOT the first slice (true for slices 1 and 2)
		var IsNotSecondSlice = float.Clamp(threeSliceSelector - 1f, 0f, 1f); // 1 if NOT the first or second slice (true only for slice 2)
	...
	public static Color FromHSV(int r, int g, int b) => FromHSV(r / 255f, g / 255f, b / 255f);
	...
	public static Color operator *(Color value, float scale) => new Color(
		value.R * scale / 255f,
		value.G * scale / 255f,
		value.B * scale / 255f,
		value.A * scale / 255f
