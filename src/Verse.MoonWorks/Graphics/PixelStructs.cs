...
public static class PixelStructs
{
	...
	private static T BitwisePack<T>(float value, int bits) where T : INumber<T> => T.CreateChecked(MathF.Truncate(((1 << bits) - 1) * System.Math.Clamp(value, 0, 1)));
	...
	public record struct R10G10B10A2Unorm(uint PackedValue)
	{
		...
		private static uint Pack(float r, float g, float b, float a) => BitwisePack<uint>(r, 10) |
	...
	public record struct B5G6R5Unorm(ushort PackedValue)
	{
		...
		private static ushort Pack(float b, float g, float r) => (ushort)(
	...
	public record struct B5G5R5A1Unorm(ushort PackedValue)
	{
		...
		private static ushort Pack(float b, float g, float r, float a) => (ushort)(
