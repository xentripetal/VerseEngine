...
public static class ImageUtils
{
	...
	/// </summary>
	public static unsafe void FreePixelData(byte* pixels)
	{
	...
	public static void FreeBufferData(IntPtr buffer)
	{
	...
	)
	{
		...
		const uint DDSD_REQ = DDSD_HEIGHT | DDSD_WIDTH;
		...
		// Format info
		var formatSize = reader.Read<uint>();
		if (formatSize != DDS_PIXFMTSIZE) {
			Logger.LogError("Bogus PIXFMTSIZE!");
			return false;
		}
		var formatFlags = reader.Read<uint>();
		var formatFourCC = reader.Read<uint>();
		var formatRGBBitCount = reader.Read<uint>();
		var formatRBitMask = reader.Read<uint>();
		var formatGBitMask = reader.Read<uint>();
		var formatBBitMask = reader.Read<uint>();
		var formatABitMask = reader.Read<uint>();

		// dwCaps "stuff"
		var caps = reader.Read<uint>();
		...
		if ((formatFlags & DDPF_FOURCC) == DDPF_FOURCC) {
			switch (formatFourCC) {
				...
				case FOURCC_DX10:
					...
					var dxgiFormat = reader.Read<uint>();
					...
					var resourceDimension = reader.Read<uint>();
					 ...
					 */
					var arraySize = reader.Read<uint>();
	...
	)
	{
		...
		if (format == TextureFormat.R32G32B32A32Float) {
