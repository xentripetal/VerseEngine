...
{
	private static readonly Dictionary<VertexElementFormat, uint> Sizes = new Dictionary<VertexElementFormat, uint> {
	...
	public static byte BoolToByte(bool b) => (byte)(b ? 1 : 0);

	public static bool ByteToBool(byte b) => b != 0;

	public static int BoolToInt(bool b) => b ? 1 : 0;

	public static bool IntToBool(int b) => b != 0;

	public static uint VertexElementFormatSize(VertexElementFormat format) => Sizes[format];
}