...
public class Queue
{
	...
	public bool GetResult(out Outcome outcome) => SDL_AsyncIO.SDL_GetAsyncIOResult(Handle, out outcome);
	...
	/// <returns>True if task has completed, false otherwise.</returns>
	public bool WaitResult(out Outcome outcome, int timeoutInMilliseconds) => SDL_AsyncIO.SDL_WaitAsyncIOResult(Handle, out outcome, timeoutInMilliseconds);
	...
	{
		private static readonly ConcurrentQueue<Queue> Queue = new ConcurrentQueue<Queue>();
