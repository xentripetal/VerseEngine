...
public class IO
{
	...
	///     Closing a file is also an asynchronous task! If a write failure were to happen during the closing process, for
	///     example, the task results will report it as usual.<br />
	///     Closing a file that has been written to does not guarantee the data has made it to physical media; it may remain in
	///     the operating system's file cache, for later writing to disk.
	...
	///     in general, but a successful result guarantees that the data has made it to physical storage. Don't use this for
	///     temporary files, caches, and unimportant data, and definitely use it for crucial irreplaceable files, like game
	///     saves.<br />
	...
	{
		private static readonly ConcurrentQueue<IO> Queue = new ConcurrentQueue<IO>();
