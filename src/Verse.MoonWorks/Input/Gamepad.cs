...
public class Gamepad
{
	...
	private readonly Dictionary<AxisButtonCode, AxisButton> AxisButtonCodeToAxisButton;
	private readonly Dictionary<SDL.SDL_GamepadAxis, Axis> EnumToAxis;

	private readonly Dictionary<SDL.SDL_GamepadButton, GamepadButton> EnumToButton;
	private readonly Dictionary<SDL.SDL_GamepadAxis, Trigger> EnumToTrigger;
	private readonly Dictionary<TriggerCode, TriggerButton> TriggerCodeToTriggerButton;

	private readonly VirtualButton[] VirtualButtons;
	...
	/// <summary>
	///     Contains a reference to an arbitrary VirtualButton that was pressed on the gamepad this frame. Useful for input
	///     remapping.
	/// </summary>
	...
	internal void Register(IntPtr handle)
	{
		...
		var joystickHandle = SDL.SDL_GetGamepadJoystick(Handle);
	...
	/// </summary>
	public bool SetVibration(float leftMotor, float rightMotor, uint durationInMilliseconds) => SDL.SDL_RumbleGamepad(
		Handle,
	...
	/// </summary>
	public VirtualButton Button(AxisButtonCode axisButtonCode) => AxisButtonCodeToAxisButton[axisButtonCode];

	/// <summary>
	///     Obtains a trigger button object given a button code.
	/// </summary>
	public VirtualButton Button(TriggerCode triggerCode) => TriggerCodeToTriggerButton[triggerCode];

	/// <summary>
	///     Obtains the axis value given an AxisCode.
	/// </summary>
	/// <returns>A value between -1 and 1.</returns>
	public float AxisValue(AxisCode axisCode) => EnumToAxis[(SDL.SDL_GamepadAxis)axisCode].Value;

	/// <summary>
	///     Obtains the trigger value given an TriggerCode.
	/// </summary>
	/// <returns>A value between 0 and 1.</returns>
	public float TriggerValue(TriggerCode triggerCode) => EnumToTrigger[(SDL.SDL_GamepadAxis)triggerCode].Value;
}